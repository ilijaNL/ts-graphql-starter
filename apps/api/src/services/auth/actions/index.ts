...
import { createSendOTP } from './send-opt';
...
export const createAuthService = (app: FastifyInstance) => {
  ...
  const jwtFactory = createJWTFactory({
    iss: 'auth',
    secret: AUTH_ENV.REFRESH_TOKEN_SECRET,
    refreshTokenInSec: parseInt(AUTH_ENV.JWT_REFRESH_TOKEN_EXPIRATION_TIME),
  });

  const accessTokenExperation = parseInt(AUTH_ENV.JWT_ACCESS_TOKEN_EXPIRATION_TIME);

  const db = getPoolFromFastify(app);
  const queryClient = createAuthDBClient(db);

  return {
    sendOTP: createSendOTP(),
    signInWithOTP: createSignWithOTP({
      dbClient: queryClient,
      createAccount: (accountId, save) => app.accountService.createAccount(null, { accountId: accountId }, save),
      signRefreshToken: jwtFactory.signRefreshToken,
    }),
    signShortToken: (token: ShortToken) => {
      return app.jwt.sign(token, {
        expiresIn: accessTokenExperation,
      });
    },
    ...
    oauth: {
      redeem: createRedeem(db, { signRefreshToken: jwtFactory.signRefreshToken }),
      ...
      callback: createCallback({
        ...
        createAccount: (accountId, save) => app.accountService.createAccount(null, { accountId }, save),
        updateAccountInfo: (props, save) =>
          app.accountService.updateAccountInfo(
            {
